...
class ProductosView:
    ...
    def build(self):
        ...
        return ft.Column([
            ...
            # Área de productos con barra de desplazamiento
            ft.Container(
                content=ft.Column(
                    [self.tabla],
                    ...
                    ft.DataCell(
                        ft.Row([
                            ...
                            ft.IconButton(
                                ...
                                icon_size=18,
                                data=p["id"],
                                on_click=lambda e: self._eliminar(
                                    int(e.control.data)
                                ),
    ...
    def _formulario(self, pid=None):
        ...
        e_nombre = ft.TextField(
            label="Nombre",
            value=prod["nombre"] if editando else "",
            width=300
        )

        e_compra = ft.TextField(
            label="Precio compra (₡)",
            value=str(prod["precio_compra"]) if editando else "",
            width=300,
            keyboard_type=ft.KeyboardType.NUMBER
        )

        e_venta = ft.TextField(
            label="Precio venta (₡)",
            value=str(prod["precio_venta"]) if editando else "",
            width=300,
            keyboard_type=ft.KeyboardType.NUMBER
        )
        ...
        e_stock = ft.TextField(
            label="Stock",
            value=str(prod["stock"]) if editando else "",
            width=300,
            keyboard_type=ft.KeyboardType.NUMBER
        )
            ...
            try:
                ...
                if editando:
                    prods.editar(
                        pid,
                        nombre=nom,
                        precio_compra=float(e_compra.value),
                        precio_venta=float(e_venta.value),
                        stock=int(e_stock.value),
        ...
        dlg = ft.AlertDialog(
            title=ft.Text(
                "Editar" if editando else "Nuevo Producto"
            ),
